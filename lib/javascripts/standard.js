...
function loadOptions(selectOptions, options) {
    ...
    var selectOptionCount = selectOptions.length;
    for (var j = 0; j < selectOptionCount; j++) {
        optionsList += "<li onmousedown='updateTouchscreenInputForSelect(this)'>" + (typeof tstLocaleWords !=
            typeof undefined ? tstLocaleWords[selectOptions[j].value.toLowerCase().trim()] : selectOptions[j].value) + "</li>\n";
    }
    optionsList += "</ul>";
    options.innerHTML = optionsList;
}
